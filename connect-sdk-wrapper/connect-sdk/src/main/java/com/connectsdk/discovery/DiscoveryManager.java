...
public class DiscoveryManager implements ConnectableDeviceListener, DiscoveryProviderListener, ServiceConfigListener {
    ...
    public DiscoveryManager(Context context, ConnectableDeviceStore connectableDeviceStore) {
        ...
        multicastLock = wifiMgr.createMulticastLock(LOG_TAG);
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (action.equals(WifiManager.NETWORK_STATE_CHANGED_ACTION)) {
                    ...
                    switch (networkInfo.getState()) {
                        ...
                        case DISCONNECTED:
                            SmartLog.w(LOG_TAG, "Network connection is disconnected");
    ...
    public void registerDeviceService(Class<? extends DeviceService> deviceClass, Class<? extends DiscoveryProvider> discoveryClass) {
        SmartLog.d(LOG_TAG, "registerDeviceService ");
        if (!DeviceService.class.isAssignableFrom(deviceClass)) {
            SmartLog.e(LOG_TAG, "Device class is not assignable!");
            return;
        }

        if (!DiscoveryProvider.class.isAssignableFrom(discoveryClass)) {
            SmartLog.e(LOG_TAG, "Discovery class is not assignable!");
            return;
        }
        ...
        try {
            ...
            SmartLog.d(LOG_TAG, "service: " + serviceId + " " + deviceClass.getName());
            deviceClasses.put(serviceId, deviceClass);

            SmartLog.d(LOG_TAG, "addDeviceFilter " + discoveryFilter);
     ...
     */
    public void rescan() {
        for (DiscoveryProvider provider : getDiscoveryProviders()) {
            provider.restart();
        }
    }
        ...
        Util.runOnUI(new Runnable() {
            ...
            public void run() {
                ...
                if (mWifi.isConnected()) {
                    ...
                    SmartLog.w(LOG_TAG, "Wifi is not connected yet");
    ...
    public void handleDeviceAdd(ConnectableDevice device) {
        SmartLog.d(LOG_TAG, "handleDeviceAdd ");
        if (!deviceIsCompatible(device))
            return;
    ...
    public void handleDeviceUpdate(ConnectableDevice device) {
        SmartLog.d(LOG_TAG, "handleDeviceUpdate");
        //modified by den
        if (deviceIsCompatible(device)) {
    ...
    public void onServiceAdded(DiscoveryProvider provider, ServiceDescription serviceDescription) {
        SmartLog.d(LOG_TAG, "Service added: " + serviceDescription.getFriendlyName() + " (" + serviceDescription.getServiceID() + ")");

        boolean deviceIsNew = !allDevices.containsKey(serviceDescription.getIpAddress());
        ConnectableDevice device = null;

        if (deviceIsNew) {
            SmartLog.d(LOG_TAG, "deviceIsNew ");
            if (connectableDeviceStore != null) {
            ...
            SmartLog.d(LOG_TAG, "device is old ");
            device = allDevices.get(serviceDescription.getIpAddress());
        }

        if (device == null) {
            SmartLog.d(LOG_TAG, "create a device ");
...
//        device.setLastSeenOnWifi(currentSSID);
    ...
    public void onServiceRemoved(DiscoveryProvider provider, ServiceDescription serviceDescription) {
        if (serviceDescription == null) {
            SmartLog.w(LOG_TAG, "onServiceRemoved: unknown service description");

            return;
        }

        SmartLog.d(LOG_TAG, "onServiceRemoved: friendlyName: " + serviceDescription.getFriendlyName());
    ...
    public void onServiceUpdated(DiscoveryProvider provider, ServiceDescription serviceDescription) {
        if (serviceDescription == null) {
            SmartLog.w(LOG_TAG, "onServiceUpdated: unknown service description");

            return;
        }

        SmartLog.d(LOG_TAG, "onServiceUpdated: friendlyName: " + serviceDescription.getFriendlyName());

        ConnectableDevice device = allDevices.get(serviceDescription.getIpAddress());

        for (DeviceService deviceService : device.getServices()) {
            if (deviceService.getServiceName().equals(serviceDescription.getServiceID())) {
                SmartLog.d(LOG_TAG, "update service description " + deviceService.getClass().getSimpleName());
                SmartLog.d(LOG_TAG, "has device: " + (serviceDescription.getDevice() != null));
                deviceService.setServiceDescription(serviceDescription);
            }
        }
    ...
    public void onServiceDiscoveryFailed(DiscoveryProvider provider, ServiceCommandError error) {
        SmartLog.w(LOG_TAG, "DiscoveryProviderListener, Service Discovery Failed");
    }

    @SuppressWarnings("unchecked")
    public void addServiceDescriptionToDevice(ServiceDescription desc, ConnectableDevice device) {
        SmartLog.d(LOG_TAG, "Adding service " + desc.getServiceID() + " to device with address " + device.getIpAddress() + " and id " + device.getId());

        Class<? extends DeviceService> deviceServiceClass = deviceClasses.get(desc.getServiceID());

        if (deviceServiceClass == null) {
            SmartLog.e(LOG_TAG, "deviceServiceClass is null! ");
            return;
        }
        ...
        if (deviceService != null) {
            SmartLog.d(LOG_TAG, "device service added ");
