...
public class CastService extends DeviceService implements MediaPlayer, MediaControl, VolumeControl, WebAppLauncher {
    ...
    public void disconnect() {
        ...
        if (mApiClient != null && mApiClient.isConnected()) {
            try {
                ...
                SmartLog.e(Util.T, "Closing application error", e);
            }
        ...
        ConnectionListener connectionListener = new ConnectionListener() {
            ...
            public void onConnected() {
                try {
                    ...
                    SmartLog.d(LOG_TAG, "stopped successfully");

                    relaunchApplication(new ApplicationConnectionResultCallback(new LaunchWebAppListener() {
                    ...
                    SmartLog.d(LOG_TAG, "Unable to stop");
                    Util.postError(listener, new ServiceCommandError(0, "Unable to stop", null));
                }
            }
        };
    ...
    private void attachMediaPlayer() {
                        ...
                        if (subscription.getTarget().equalsIgnoreCase(PLAY_STATE)) {
                            for (int i = 0; i < subscription.getListeners().size(); i++) {
                                ...
                                if (mMediaPlayer != null && mMediaPlayer.getMediaStatus() != null) {
                                    ...
                                    SmartLog.d(LOG_TAG, "received status " + status.name());
        ...
        if (mApiClient != null) {
            try {
                ...
                SmartLog.w(Util.T, "Exception while creating media channel", e);
            }
        }
    }
    ...
    private void detachMediaPlayer() {
        if ((mMediaPlayer != null) && (mApiClient != null)) {
            try {
                ...
                SmartLog.w(Util.T, "Exception while launching application", e);
            }
        }
        mMediaPlayer = null;
    }
    ...
    public void displayImage(String url, String mimeType, String title,
        ...
        SmartLog.d(LOG_TAG, "displayImage " + url);
    ...
    public void displayImage(MediaInfo mediaInfo, LaunchListener listener) {
        SmartLog.d(LOG_TAG, "displayImage ");
    ...
    private void playMedia(String url, SubtitleInfo subtitleInfo, String mimeType, String title,
        ...
        SmartLog.d(LOG_TAG, "playMedia " + url);
        ...
        if (subtitleInfo != null) {
            SmartLog.d(LOG_TAG, "subtitles " + subtitleInfo.getUrl() + " " + subtitleInfo.getMimeType());
    ...
    private void loadMedia(com.google.android.gms.cast.MediaInfo mediaInformation,
        ...
        try {
            mMediaPlayer.load(mApiClient, mediaInformation, true).setResultCallback(new ResultCallback<MediaChannelResult>() {
                ...
                public void onResult(MediaChannelResult result) {
                    Status status = result.getStatus();
                    SmartLog.d(LOG_TAG, "loadMedia status " + status.isSuccess() + " " + status.getStatusMessage());
                    if (status.isSuccess()) {
    ...
    public void launchWebApp(final String webAppId, final boolean relaunchIfRunning, final WebAppSession.LaunchListener listener) {
        SmartLog.d(LOG_TAG, "launchWebApp " + webAppId + ", relaunchIfRunning " + relaunchIfRunning);
                        ...
                        mCastClient.joinApplication(mApiClient).setResultCallback(new ResultCallback<Cast.ApplicationConnectionResult>() {
                            ...
                            public void onResult(ApplicationConnectionResult result) {
                                ...
                                if (result.getStatus().isSuccess() &&
                                    ...
                                    SmartLog.e(LOG_TAG, "joinApplication was not completed successfully!");

                                    LaunchOptions options = new LaunchOptions();
                                    options.setRelaunchIfRunning(true);

                                    try {
                                        SmartLog.d(LOG_TAG, "launchApplication with cast client ");
                                        ...
                                        SmartLog.e(LOG_TAG, "launchApplication " + e.getMessage());
                                        Util.postError(listener, new ServiceCommandError(0, "Unable to launch", null));
                                    }
                                }
                            }
    ...
    private class CastListener extends Cast.Listener {
        ...
        public void onApplicationDisconnected(int statusCode) {
            SmartLog.d(LOG_TAG, "Cast.Listener.onApplicationDisconnected: " + statusCode);
                ...
                public void onConnected() {
                    if (mApiClient != null) {
                        ...
                        try {
                            ...
                            SmartLog.e(Util.T, "Error in onApplicationStatusChanged", e);
                        }
                    }
                }
            };
    ...
    private class ConnectionCallbacks implements GoogleApiClient.ConnectionCallbacks {
        ...
        public void onConnectionSuspended(final int cause) {
            SmartLog.d(LOG_TAG, "ConnectionCallbacks.onConnectionSuspended");
        ...
        public void onConnected(Bundle connectionHint) {
            SmartLog.d(LOG_TAG, "ConnectionCallbacks.onConnected, wasWaitingForReconnect: " + mWaitingForReconnect);
            ...
            if (mApiClient != null && mApiClient.isConnected()) {
                try {
                    ...
                    SmartLog.e(Util.T, "join application error", e);
                }
            }
        }
        ...
        private void joinFinished() {
            SmartLog.d(LOG_TAG, "joinFinished ");
            ...
            try {
                ...
                final ApplicationConnectionResultCallback webAppLaunchCallback =
                        new ApplicationConnectionResultCallback(new LaunchWebAppListener() {
                            ...
                            @Override
                            public void onSuccess(final WebAppSession webAppSession) {
                                SmartLog.d(LOG_TAG, "launchApplication onResult");
                            ...
                            @Override
                            public void onFailure(ServiceCommandError error) {
                                SmartLog.d(LOG_TAG, "launchApplication onFailure");
                                mConnectionFailedListener.onConnectionFailed(null);
                            }
                        });

                mCastClient.launchApplication(mApiClient, applicationID, options).setResultCallback(webAppLaunchCallback);
            } catch (Exception e) {
                SmartLog.e(LOG_TAG, "launchApplication " + e.getMessage());
                mConnectionFailedListener.onConnectionFailed(null);
            }
    ...
    private void relaunchApplication(ApplicationConnectionResultCallback callback) throws CastClientException {
        LaunchOptions options = new LaunchOptions();
        options.setRelaunchIfRunning(true);

        mCastClient.launchApplication(mApiClient, applicationID, options).setResultCallback(callback);
    }

    private class ConnectionFailedListener implements GoogleApiClient.OnConnectionFailedListener {
        @Override
        public void onConnectionFailed(final ConnectionResult result) {
            SmartLog.d(LOG_TAG, "ConnectionFailedListener.onConnectionFailed " + (result != null ? result : ""));
                ...
                public void run() {
                    if (listener != null) {
                        ...
                        if (result != null) {
                            ...
                            errorCode = ServiceCommandError.SERVICE_UNAVAILABLE;
                        }

                        ServiceCommandError error = new ServiceCommandError(errorCode, "Failed to connect to Google Cast device", result);
    ...
    public void getPlayState(PlayStateListener listener) {
        ...
        if (mMediaPlayer != null && mMediaPlayer.getMediaStatus() != null) {
            PlayStateStatus status = PlayStateStatus.convertPlayerStateToPlayStateStatus(mMediaPlayer.getMediaStatus().getPlayerState());
            SmartLog.d(LOG_TAG, "get play status " + status.name() + " " + mMediaPlayer.getMediaStatus());
            Util.postSuccess(listener, status);
        } else {
            Util.postError(listener, new ServiceCommandError(0, "There is no media currently available", null));
        }
