...
public class DIALService extends DeviceService implements Launcher {
    ...
    private static final String LOG_TAG = DIALService.class.getSimpleName();
    ...
    public void launchAppWithInfo(final AppInfo appInfo, final Object params, final AppLaunchListener listener) {
        SmartLog.d(LOG_TAG, "launchAppWithInfo " + appInfo.getId() + " " + appInfo.getName());
    ...
    @Override
    public void closeApp(String appId, final ResponseListener<Object> listener) {
        SmartLog.i(LOG_TAG, "closeApp " + appId);
        LaunchSession launchSession = LaunchSession.launchSessionForAppId(appId);
        launchSession.setSessionId("/apps/" + appId + "/run");
        launchSession.setService(DIALService.this);
        launchSession.setSessionType(LaunchSessionType.App);

        closeApplication(launchSession, listener);
//
//        getAppState(launchSession, new AppStateListener() {
//
//            @Override
...
//            @Override
//            public void onError(ServiceCommandError error) {
//                SmartLog.e(LOG_TAG, "close app error " + error.getMessage());
//                Util.postError(listener, error);
//            }
//        });
    }
    ...
    public void closeApp(final LaunchSession launchSession, final ResponseListener<Object> listener) {
        SmartLog.i(LOG_TAG, "closeApp " + launchSession.getSessionId());
        closeApplication(launchSession, listener);
//
//        getAppState(launchSession, new AppStateListener() {
//
...
//        });
    ...
    private void closeApplication(LaunchSession launchSession, ResponseListener<Object> listener) {
        String uri;
        ...
        SmartLog.d(LOG_TAG, "close app with uri " + uri);
    ...
    public void launchNetflix(final String contentId, AppLaunchListener listener) {
        ...
        if (contentId != null && contentId.length() > 0) {
            try {
                ...
                SmartLog.e(Util.T, "Launch Netflix error", e);
            }
        }
    ...
    public void getAppState(LaunchSession launchSession, final AppStateListener listener) {
        SmartLog.i(LOG_TAG, "getAppState ");
...
//                        listener.onAppStateSuccess(state);
            ...
            @Override
            public void onError(ServiceCommandError error) {
                SmartLog.e(LOG_TAG, "getAppState error " + error.getMessage());
                Util.postError(listener, error);
            }
        };
        ...
        SmartLog.d(LOG_TAG, "launch app with url " + uri);
    }

    @Override
    public void getAppState(String getAppId, final AppStateListener listener) {
        SmartLog.i(LOG_TAG, "getAppState ");
...
//                        listener.onAppStateSuccess(state);
            ...
            @Override
            public void onError(ServiceCommandError error) {
                SmartLog.d(LOG_TAG, "getAppState onError " + error);
                Util.postError(listener, error);
            }
        };

        String uri = requestURL(getAppId);

        SmartLog.d(LOG_TAG, "requestURL " + uri);
...
//        mServiceReachability.start();
    ...
    private void probeForAppSupport() {
        if (serviceDescription.getApplicationURL() == null) {
            SmartLog.d(Util.T, "unable to check for installed app; no service application url");
            return;
        }
