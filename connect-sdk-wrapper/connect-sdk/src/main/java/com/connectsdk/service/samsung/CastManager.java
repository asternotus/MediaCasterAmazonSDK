...
import com.connectsdk.service.samsung.msgs.ErrorMessage;
import com.connectsdk.service.samsung.msgs.MessageBase;
import com.connectsdk.service.samsung.msgs.MessageType;
import com.connectsdk.service.samsung.statemachine.StateMachine;
...
import com.samsung.multiscreenfix.Search;
import com.samsung.multiscreenfix.Search.OnBleFoundListener;
import com.samsung.multiscreenfix.Search.OnServiceFoundListener;
import com.samsung.multiscreenfix.Search.OnServiceLostListener;
 ...
 *
 * @author m.gajewski
 * @see com.connectsdk.service.samsung.statemachine.StateMachine
 * @see com.connectsdk.service.samsung.model.CastStates
 */
public class CastManager extends StateMachine<CastStates> {
     ...
     * @see com.connectsdk.service.samsung.RemoteController
     */
    ...
    public void wakeDevice(String mac, final Uri uri, final SimpleCallback<Service> callback) {
        wakeDevice(mac, uri, 60000, callback);
    }
     ...
     * connection.
     *
     * @param remote ConnectData object with data needed to make connection.
     * @see com.connectsdk.service.samsung.model.ConnectData
     */
     ...
     *
     * @param remote  ConnectData object with data needed to make connection.
     * @param timeout Time for connection. After that time when not connected error
     *                will appear
     * @see com.connectsdk.service.samsung.model.ConnectData
     */
    public void connect(ConnectData remote, int timeout) {
        if (getCurrentState() == CastStates.IDLE) {
            ...
            if (mRemoteApp == null) { // try to get application from service if it
                ...
                if (startArgs != null) {
                    SmartLog.d(LOG_TAG, "createApplication with args ");
                    mRemoteApp = service.createApplication(remote.getAppId(), remote.getChannelId(),
                            startArgs);
                } else {
                    SmartLog.d(LOG_TAG, "createApplication");
                    mRemoteApp = service.createApplication(remote.getAppId(), remote.getChannelId());
                }
            }
            if (mRemoteApp != null) {
                SmartLog.d(LOG_TAG, "remote app detected ");
            ...
            if (getCurrentState() == CastStates.READY) {
                gotoState(CastStates.CONNECTING, null);
                gotoState(CastStates.CONNECTED, null);
                gotoState(CastStates.READY, null);
            } else {
                SmartLog.d(LOG_TAG, "Current state " + getCurrentState().name());
                SmartLog.wtf(LOG_TAG, "WTF connect 202 error! ");
     ...
     * device.
     *
     * @param exitRemote true for close remote application or false otherwise.
     */
    public void disconnect(boolean exitRemote) {
        SmartLog.d(LOG_TAG, "disconnect " + exitRemote);
        if (!mDisconnectRequested) {
            mDisconnectRequested = true;
            if (mRemoteApp != null && mClientObject != null) {
                SmartLog.d(LOG_TAG, "mRemoteApp and mClientObject are valid! ");
                if (mDisconnectResult != null) {
                    SmartLog.d(LOG_TAG, "mDisconnectResult cancel");
                ...
                SmartLog.e(LOG_TAG, "Disconnecting error. Client wasn't connected. ");
                FCError libError = new FCError(211, "Disconnecting error. Client wasn't connected.");
                synchronized (mFCErrorListeners) {
                    for (OnFCErrorListener listener : mFCErrorListeners) {
                        listener.OnError(libError);
                    }
                }
     ...
     *
     * @param type MessageType enumerator describing type of message
     * @param msg  Message object inherited from MessageBase
     * @see com.connectsdk.service.samsung.msgs.MessageType
     * @see com.connectsdk.service.samsung.msgs.MessageBase
     */
     ...
     * for receiving.
     *
     * @param customEvent Object of your class inherited from CustomEvent and described
     *                    event you want send
     * @see com.connectsdk.service.samsung.msgs.CustomEvent
     */
     ...
     *
     * @param serviceAdapter ArrayAdapter<Service> object which will be filled in discovery
     *                       process.
     */
     ...
     * discovery process.
     *
     * @param listener Your {@link OnDiscoveryListener}
     */
     ...
     *
     * @param listener {@link OnDiscoveryListener} to remove
     */
     ...
     * discovery process.
     *
     * @param listener Your {@link OnBLEDiscoveryListener}
     */
     ...
     *
     * @param listener {@link OnBLEDiscoveryListener} to remove
     */
     ...
     * from remote device.
     *
     * @param listener Your {@link OnTVMessageListener} listener.
     */
     ...
     *
     * @param listener {@link OnTVMessageListener} to remove.
     */
     ...
     *
     * @param listener Your {@link OnFCErrorListener} listener.
     */
     ...
     *
     * @param listener {@link OnFCErrorListener} to remove.
     */
    public void removeOnFCErrorListener(OnFCErrorListener listener) {
     ...
     * CustomEvent class.
     *
     * @param customEvent Object of your CustomEvent class (has to inherit from
     *                    CustomEvent
     ...
     *
     * @see com.connectsdk.service.samsung.msgs.MessageType
     * @see com.connectsdk.service.samsung.MessageListener
     */
