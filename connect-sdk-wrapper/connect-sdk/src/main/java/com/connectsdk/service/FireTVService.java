 ...
 */
public class FireTVService extends DeviceService implements MediaPlayer, MediaControl, VolumeControl {

    private static final String LOG_TAG = FireTVService.class.getSimpleName();
    private static final Float VOLUME_UP_VALUE = 0.1f; //10% of max volume //TODO 5% and test
    private static final Float VOLUME_DOWN_VALUE = 0.1f; //10% of max volume //TODO 5% and test
    ...
    private static final String COMMAND_TYPE_KEY = "custom_command_type";
    private static final String COMMAND_TYPE_SUBTITLES = "subtitles";
    private static final String MAIN_ACTIVITY_LAUNCH_MODE_PROGRESS_SCREEN_KEY = "show_progress_bar";
    private static final boolean MAIN_ACTIVITY_LAUNCH_MODE_PROGRESS_SCREEN_SHOW = true;
    private static final boolean MAIN_ACTIVITY_LAUNCH_MODE_PROGRESS_SCREEN_HIDE = false;
    ...
    public void setLauncher(Launcher launcher, String receiverAppId) {
        SmartLog.d(LOG_TAG, "setLauncher " + receiverAppId);
    ...
    public void playMedia(String url, String mimeType, String title, String description,
                          String iconSrc, boolean shouldLoop, final LaunchListener listener) {
        ...
        setMediaSource(mediaInfo, new LaunchListener() {
            @Override
            ...
            public void onError(ServiceCommandError error) {
                Log.e(LOG_TAG, "Could not cast media! " + error.getMessage());
                if (listener != null) {
                    listener.onError(ServiceCommandError.getError(ServiceCommandError.FIRE_TV_NEEDS_REBOOT));
                }
            }
        }, true, true);
    }
    ...
    public void displayImage(MediaInfo mediaInfo, LaunchListener listener) {
        setMediaSource(mediaInfo, listener, false, false);
    }
    ...
    @Override
    public void showProgressScreen(final ResponseListener<Object> responseListener) {
        SmartLog.d(LOG_TAG, "showProgressScreen "+receiverAppId);

        final JSONObject launchParams = new JSONObject();
        try {
            launchParams.put(MAIN_ACTIVITY_LAUNCH_MODE_PROGRESS_SCREEN_KEY, MAIN_ACTIVITY_LAUNCH_MODE_PROGRESS_SCREEN_SHOW);
        } catch (JSONException e) {
            e.printStackTrace();
        }

        AppInfo appInfo = new AppInfo();
        appInfo.setName("MegaCast");
        appInfo.setId(receiverAppId);
        launcher.launchAppWithInfo(appInfo, launchParams.toString(), new Launcher.AppLaunchListener() {
            @Override
            public void onSuccess(LaunchSession object) {
                responseListener.onSuccess(object);
            }
    ...
    @Override
    public void hideProgressScreen(final ResponseListener<Object> responseListener) {
        SmartLog.d(LOG_TAG, "hideProgressScreen "+receiverAppId);

        final JSONObject launchParams = new JSONObject();
        try {
            launchParams.put(MAIN_ACTIVITY_LAUNCH_MODE_PROGRESS_SCREEN_KEY, MAIN_ACTIVITY_LAUNCH_MODE_PROGRESS_SCREEN_HIDE);
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public void volumeUp(final ResponseListener<Object> listener) {
        SmartLog.d(LOG_TAG, "volumeUp ");
        getVolume(new VolumeListener() {
            @Override
            public void onSuccess(Float level) {
                SmartLog.d(LOG_TAG, "got volume level " + level);
                float volume = level + VOLUME_UP_VALUE;

                setVolume(volume, listener);
            }

            @Override
            public void onError(ServiceCommandError error) {
                if (listener != null) {
                    listener.onError(new FireTVServiceError("Could not get volume level!"));
                }
            }
        });
    }
    ...
    public void volumeDown(final ResponseListener<Object> listener) {
        ...
        getVolume(new VolumeListener() {
            ...
            public void onSuccess(Float level) {
                SmartLog.d(LOG_TAG, "got volume level " + level);
                float volume = level - VOLUME_DOWN_VALUE;
    ...
    public void setVolume(float volume, ResponseListener<Object> listener) {
        final String error = "Error setting volume";
        RemoteMediaPlayer.AsyncFuture<Void> asyncFuture = null;
        try {
            asyncFuture = remoteMediaPlayer.setVolume(volume);
            handleVoidAsyncFuture(listener, asyncFuture, error);
        } catch (Exception e) {
            Util.postError(listener, new FireTVServiceError(e.getMessage() != null ? e.getMessage() : error, e));
        }
    }

    @Override
    public void getVolume(final VolumeListener listener) {
        final String error = "Error getting volume level";
        RemoteMediaPlayer.AsyncFuture<Double> asyncFuture;
        try {
            asyncFuture = remoteMediaPlayer.getVolume();
            handleAsyncFuture(new ResponseListener<Double>() {
                @Override
                public void onSuccess(Double object) {
                    if (listener != null) {
                        listener.onSuccess(object.floatValue());
                    }
                }
    ...
    public void setMute(boolean isMute, ResponseListener<Object> listener) {
        final String error = "Error setting mute " + isMute;
        RemoteMediaPlayer.AsyncFuture<Void> asyncFuture = null;
        try {
            asyncFuture = remoteMediaPlayer.setMute(isMute);
            handleVoidAsyncFuture(listener, asyncFuture, error);
        } catch (Exception e) {
            Util.postError(listener, new FireTVServiceError(error, e));
        }
    ...
    public void getMute(final MuteListener listener) {
        final String error = "Error getting mute status";
        RemoteMediaPlayer.AsyncFuture<Boolean> asyncFuture;
        try {
            asyncFuture = remoteMediaPlayer.isMute();
            handleAsyncFuture(new ResponseListener<Boolean>() {
                @Override
    ...
    public void changeSubtitles(SubtitleInfo info, ResponseListener<Object> listener) {
        final String error = "Could not add subtitles";
        try {
            JSONObject json = new JSONObject();
            json.put(COMMAND_TYPE_KEY, COMMAND_TYPE_SUBTITLES);

            JSONArray tracksArray = new JSONArray();
            JSONObject trackObj = new JSONObject();
            trackObj.put(META_SRC, info.getUrl());
            ...
            remoteMediaPlayer.sendCommand(json.toString());
    ...
    public ServiceSubscription<PlayStateListener> subscribePlayState(
        ...
        if (playStateSubscription == null) {
            SmartLog.d(LOG_TAG, "playStateSubscription add status listener");
    ...
    public void unSubscribePlayState(PlayStateListener listener) {
        ...
        if (playStateSubscription != null && playStateSubscription.getListeners().contains(listener)) {
            ...
            remoteMediaPlayer.removeStatusListener(playStateSubscription);
            playStateSubscription = null;
        }
    }
    ...
    private String getMetadata(MediaInfo mediaInfo)
        ...
        if (mediaInfo.getSubtitleInfo() != null) {
            ...
            trackObj.put(META_SRCLANG, "eng");
//            trackObj.put(META_SRCLANG, language == null ? "" : language);
            tracksArray.put(trackObj);
            json.put(META_TRACKS, tracksArray);

            SmartLog.d(LOG_TAG, "track \n" + mediaInfo.getSubtitleInfo().getUrl());
            SmartLog.d(LOG_TAG, "tracks \n" + tracksArray.toString());
        }

        String extra = mediaInfo.getExtra() != null ? mediaInfo.getExtra().toString() : "";

        SmartLog.d(LOG_TAG, "extras: " + extra);

        json.put("extra", mediaInfo.getExtra());
    ...
    private void setMediaSource(final MediaInfo mediaInfo, final LaunchListener listener, final boolean waitForPlayState, final boolean retry) {
        SmartLog.d(LOG_TAG, "[play] setMediaSource ");

        final String error = "Could not play media file";
        RemoteMediaPlayer.AsyncFuture<Void> asyncFuture = null;
        try {
            ...
            SmartLog.e(LOG_TAG, "[play] Exception in setMediaSource " + e.getMessage());
            Util.postError(listener, new FireTVServiceError(error, e));
            return;
        }

        if (asyncFuture != null) {
            asyncFuture.getAsync(new RemoteMediaPlayer.FutureListener<Void>() {
                @Override
                public void futureIsNow(Future<Void> future) {
                    try {
                        ...
                        if (waitForPlayState) {
                            waitForPlayState(mediaInfo, listener, retry, error);
                        } else {
                            Util.postSuccess(listener, createMediaLaunchObject());
                        }
    ...
    private void waitForPlayState(final MediaInfo mediaInfo, final LaunchListener listener, final boolean retry, final String error) {
        Log.d(LOG_TAG, "[play] waitForPlayState");
        subscribePlayState(new MediaControl.PlayStateListener() {
            ...
            public void onSuccess(MediaControl.PlayStateStatus object) {
                ...
                if (object == MediaControl.PlayStateStatus.Playing) {
                    SmartLog.d(LOG_TAG, "[play] successful media launch! ");
                    listener.onSuccess(createMediaLaunchObject());
                    playStateSubscription.removeListener(this);
                } else if (object == PlayStateStatus.Error) {
                    if (retry) {
                        Log.e(LOG_TAG, "Could not play media file, retry!");
                        setMediaSource(mediaInfo, listener, true, false);
                        playStateSubscription.removeListener(this);
                    } else {
                        listener.onError(new FireTVServiceError(error));
                        playStateSubscription.removeListener(this);
                    }
                }
            }
